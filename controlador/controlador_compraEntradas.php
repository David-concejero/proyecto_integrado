<?php
  ...
  require_once("../modelo/php-mailer/PHPMailerAutoload.php");
  require_once("../modelo/vendor/autoload.php");
  ini_set('display_errors', 1);
  ini_set('display_startup_errors', 1);
  error_reporting(E_ALL);

  if(isset($_POST['comprarEntradas'])){ // AKI
    $entradas_compradas = $_POST['numeroEntradas'];
    $precio = $_POST['precioEntradas'];
    $temp = explode('€',trim($precio));
    $precio_entradas = $temp[0];
    $sala_elegida = $_POST['campo_idsala'];
    $id_pelicula = $_POST['campo_idpelicula'];
    $dia_pelicula = $_POST['campo_dia_cartelera'];
    $hora_pelicula = $_POST['campo_hora'];
    $tarifa = $_POST['campo_idtarifa'];
    $id_usuario = $_SESSION["ID_USUARIO"];

    if(isset($_POST['adjuntarFactura'])){
      require_once("../vista/generarFactura.php");
    }
    require_once("../vista/generarTicket.php");
    require_once("../vista/enviarCorreo.php");

    $select = "INSERT INTO VENTA_ENTRADAS VALUES(NULL,$id_pelicula,$id_usuario,'$dia_pelicula','$hora_pelicula',$sala_elegida,$tarifa,$entradas_compradas)";
    $consulta = conectarBaseDatos::hacerConsulta($select);
    if(isset($_POST['adjuntarFactura'])){
      unlink("../vista/facturas/factura.pdf"); // elimino la factura despues de enviarla.
    }
    unlink("../vista/facturas/ticket.pdf");
    header('Location: controlador_compraEntradas.php?mostrar=agradecimiento'); // cambiar
  }
...
<head>
  ...
  <!-- CSS y JS -->
  <link rel="stylesheet" type="text/css" href="../vista/css/controlador_compraEntradas.css">
...
<script type="text/javascript">
  $(function(){
    var nombre_entrada = $("tbody tr td").eq(1).text();
    var precio = $("span.hidden").text();
    var cantidad = $("input#entradasComprar.form-control").val();
    var cantidad_pagar = precio*cantidad;
    $("input#PrecioEntradas.form-control").val(cantidad_pagar+"€");

    $("input#entradasComprar.form-control").bind('keyup mouseup', function () {
      var precio = $("span.hidden").text();
      var cantidad = $("input#entradasComprar.form-control").val();

      // Si la tarifa contiene las letras 2x1, siempre y cuando se compren
      // 2 entradas, se cobrará como si fuese una.
      if(nombre_entrada.includes("2x1") && cantidad == 2){
        var cantidad_pagar = precio;
      }
      else{
        var cantidad_pagar = precio*cantidad;  
      }
...
<div class="container">
  <div class="row">
    ...
    <?php
    if(isset($_POST['reservarEntradas'])){
      $sala_elegida = $_POST['sala_elegida'];
      $id_pelicula = $_POST['campo_idpelicula'];
      $dia_pelicula = $_POST['campo_diapelicula'];
      $hora_pelicula = $_POST['campo_horapelicula'];

      $select = "SELECT * FROM CARTELERA WHERE DIA='$dia_pelicula' AND ID_PELICULA='$id_pelicula'";
      $consulta = conectarBaseDatos::hacerConsulta($select);
      require_once("../vista/mostrarFicha.php");

      $select = "SELECT * FROM PELICULAS WHERE ID_PELICULA='$id_pelicula'";
      $consulta = conectarBaseDatos::hacerConsulta($select);
      require_once("../vista/mostrarFormularioPago.php");
    }
    elseif(isset($_GET['mostrar'])){
      require_once("../vista/mostrarAgradedecimiento.php");
    }
    else{
